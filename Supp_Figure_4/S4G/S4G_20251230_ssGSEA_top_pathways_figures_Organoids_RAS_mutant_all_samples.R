################################################################################
# Figure Panel : Supplementary Figure 4G
# Description  : ssGSEA in COLOSSUS organoid (tumoroid) models – RAS-mutant
#                (all replicates, mean per sample).
# Input data   : GSE204805 VST (LMO organoids, RAS-mutant subset)
#                kegg_legacy gmt
# Output       : S4G_..._LMO_organoids_RAS_mutant_ssGSEA.pdf
...
out_gct   <- "GSE204805_vsd_LMO_organoids_RAS_mutant_GSVA_ssGSEA_results_KEGG_Legacy.gct"
...
select <- read.delim("20251225_LMO_KRAS_NRAS_mutatant_names.txt", stringsAsFactors = FALSE)

#RAS mutant samples used in the manuscript
samples <- substr(select$Genealogy.ID,1,10)
...
#write.table(expr_mat,"20251230_GSE204805_vsd_LMO_organoids_RAS_mutant_RNAseq_data.txt", sep = "\t", quote = FALSE)
...
#write.table(GSVA_RNAseq_m, "GSE204805_vsd_LMO_organoids_RAS_mutant_GSVA_ssGSEA_top_pathways_Pathway_Distribution_data_selected_samples_average.txt", sep="\t", quote=FALSE, row.names=TRUE)
...
cat(rep("=", 80), "\n", sep = "")
print(pathway_data[, c("Display_Name")])
...
# Save Panel B
# Save figure to PDF.
ggsave("S4G_GSE204805_vsd_LMO_organoids_RAS_mutant_GSVA_ssGSEA_top_pathways_Pathway_Distribution_data_selected_samples_average_violin_plot.pdf", 
...
# Save Panel B
ggsave("S4G_GSE204805_vsd_LMO_organoids_RAS_mutant_GSVA_ssGSEA_top_pathways_Pathway_Distribution_data_selected_samples_average_boxplot.pdf", 
...
write.table(long_data[,1:3],"S4G_GSE204805_vsd_LMO_organoids_RAS_mutant_GSVA_ssGSEA_top_pathways_selected_metabolites_enriched_violin_plot_GSVA_ssGSEA_ordered_scores.txt", sep = "\t", quote = FALSE, row.names = FALSE)
