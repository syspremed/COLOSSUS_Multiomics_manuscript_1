################################################################################
# Figure Panel : Supplementary Figure 4H
# Description  : ssGSEA in COLOSSUS organoid models – RAS-wildtype.
#                Mirror of S4G for the wildtype arm.
# Input data   : GSE204805 VST (LMO organoids, RAS-wildtype subset)
#                kegg_legacy gmt
# Output       : S4H_..._LMO_organoids_RAS_wildtype_ssGSEA.pdf
...
out_gct   <- "GSE204805_vsd_LMO_organoids_RAS_wildtype_GSVA_ssGSEA_results_KEGG_Legacy.gct"
...
select <- read.delim("20251225_LMO_KRAS_NRAS_wildtype_names.txt", stringsAsFactors = FALSE)

#RAS mutant samples used in the manuscript
samples <- substr(select$sGenealogy.ID,1,10)
...
#write.table(expr_mat,"20260322_GSE204805_vsd_LMO_organoids_RAS_wt_RNAseq_data.txt", sep = "\t", quote = FALSE)
...
#write.table(GSVA_RNAseq_m, "GSE204805_vsd_LMO_organoids_RAS_wildtype_GSVA_ssGSEA_top_pathways_Pathway_Distribution_data_selected_samples_average.txt", sep="\t", quote=FALSE, row.names=TRUE)
...
# Save Panel B
# Save figure to PDF.
ggsave("S4H_GSE204805_vsd_LMO_organoids_RAS_wildtype_GSVA_ssGSEA_top_pathways_Pathway_Distribution_data_selected_samples_average_violin_plot.pdf", 
...
# Save Panel B
ggsave("S4H_GSE204805_vsd_LMO_organoids_RAS_wildtype_GSVA_ssGSEA_top_pathways_Pathway_Distribution_data_selected_samples_average_boxplot.pdf", 
...
write.table(long_data[,1:3],"S4H_GSE204805_vsd_LMO_organoids_RAS_wildtype_GSVA_ssGSEA_top_pathways_selected_metabolites_enriched_violin_plot_GSVA_ssGSEA_ordered_scores.txt", sep = "\t", quote = FALSE, row.names = FALSE)
