################################################################################
# Figure Panel : Supplementary Figure 4B
# Description  : ssGSEA validation in public COAD cohort (GSE89076 – Agilent
#                microarray). Reads matched expression/metabolomics data and
#                generates scatter + boxplot vs MetaboAnalyst-enriched pathways.
# Input data   : GSE89076.Agilent8x60K.Matched_expression_tumours_matched.csv
#                Matched metabolomics file; kegg_legacy gmt
# Output       : S4B_..._Combined_Panels_AB.pdf
...
#    "KEGG_FATTY_ACID_BIOSYNTHESIS",
  ...
  Metabolomics_pvalue = c(
    0.0015787,   # Alanine, aspartate and glutamate
    0.015893,  # Glyoxylate and dicarboxylate
#    0.00022024,  # Biosynthesis of unsaturated fatty acids
    0.031433,  # Butanoate
    0.0019178,   # Citrate cycle (TCA)
    0.0033349,    # Arginine and proline
    0.67797,    # Pyruvate
    0.003834,   # Glycine, serine and threonine (NEW)
   # 0.03034,     # Fatty acid biosynthesis (NEW)
    3.4258e-07      # Purine metabolism (NEW)
  ),
  Metabolomics_FDR = c(
    0.030685,  # Alanine, aspartate and glutamate
    0.079464,   # Glyoxylate and dicarboxylate
#    0.0044048,   # Biosynthesis of unsaturated fatty acids
    0.13235,   # Butanoate
    0.030685,    # Citrate cycle (TCA)
    0.043817,    # Arginine and proline
    1,    # Pyruvate
    0.043817,    # Glycine, serine and threonine (NEW)
   # 0.29065,     # Fatty acid biosynthesis (NEW)
    2.7407e-05       # Purine metabolism (NEW)
  ),
  Impact = c(
    0.51042,     # Alanine, aspartate and glutamate
    0.11,        # Glyoxylate and dicarboxylate
 #   0,           # Biosynthesis of unsaturated fatty acids
    0.03175,           # Butanoate
    0.21812,       # Citrate cycle (TCA)
    0,     # Arginine and proline
    0,     # Pyruvate
    0.2505,     # Glycine, serine and threonine (NEW)
 #   0.01473,     # Fatty acid biosynthesis (NEW)
    0.20922      # Purine metabolism (NEW)
  ),
  Total_compounds = c(28, 32, 15, 20, 36, 23, 33, 70),
  Hits = c(6, 5, 3, 5, 6, 1, 6, 15),
...
expr_file <- "Supplementary data/GSE89076/GSE89076.Agilent8x60K.Matched_expression_tumours_matched.csv"

gmt_file  <- "~/Downloads/c2.cp.kegg_legacy.v2024.1.Hs.symbols.gmt"

out_gct   <- "Supplementary data/GSE89076/GSE89076.Agilent8x60K.Matched_expression_tumours_matched_GSVA_ssGSEA_results_KEGG_Legacy.gct"
...
#write.table(GSVA_RNAseq_m, "GSE89076.Agilent8x60K.Matched_expression_tumours_matched_Metabolanalyst_FDR_ssGSEA_top_pathways_Pathway_Distribution_data.txt", sep="\t", quote=FALSE, row.names=TRUE)
...
# Save Panel A
## Save figure to PDF.
ggsave("20251217_GSE89076.Agilent8x60K.Matched_expression_tumours_matched_Metabolanalyst_FDR_ssGSEA_top_pathways_Scatter.pdf", 
...
# Save Panel B
ggsave("S4B_20251217_GSE89076.Agilent8x60K.Matched_expression_tumours_matched_Metabolanalyst_FDR_ssGSEA_top_pathways_Pathway_Distribution_Boxplots.pdf", 
  panel_b_alt, width = 14, height = 13, dpi = 300)
cat("✓ Panel B saved: 20251210_COLOSSUS_Metabolanalyst_FDR_ssGSEA_top_pathways_Pathway_Distribution_Boxplots.pdf\n")

write.table(long_data[,1:3],"S4B_20251217_GSE89076.Agilent8x60K.Matched_expression_tumours_matched_Metabolanalyst_FDR_ssGSEA_top_pathways_selected_metabolites_enriched_violin_plot_GSVA_ssGSEA_ordered_scores.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
ggsave("S4B_20251217_GSE89076.Agilent8x60K.Matched_Metabolanalyst_FDR_ssGSEA_top_pathways_Pathway_Distribution_scatter_Boxplots_ssGSEA_Combined_Panels_AB.pdf", final_plot, 
